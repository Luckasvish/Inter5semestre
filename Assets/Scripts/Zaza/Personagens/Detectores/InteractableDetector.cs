...
public class InteractableDetector : MonoBehaviour
{
     ...
     void InteractionDetection()
    {
        ...
        if(Physics.Raycast(detection, out interactable, manager.detectionDistance ,InteractableLayer)) /// (Se tiver uma detecção)
        {
            ...
            {
                if(manager.interactionOBJ == null)    /// (Se ainda não houver um interagível no manager)
                {
                    manager.SetDetection(interactable.transform.GetComponent<_InteractionOBJ>());  /// retorna a interação ao manager;
                }
